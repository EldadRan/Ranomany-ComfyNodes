...
export class CameraWidget {
  ...
  public generatePrompt(): string {
    // Keep these zones in sync with nodes/camera_angle/node.py.
    ...
    } else if (this.state.distance < 6.0) {
    ...
    return `Change the camera to a ${distance} from a ${vDirection}, ${hDirection} of the same subject. ` +
      `Preserve identity, materials, and lighting — only change the camera angle and framing.`
  }
